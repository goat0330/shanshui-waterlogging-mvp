...
- 本轮 `StatusPanel` 目标参考：`C:\Users\WangChi\AppData\Local\Temp\codex-clipboard-94f3bfbf-5f6b-4292-9aa2-a91ed80e0342.png`
- 本轮对照截图（旧实现）：`C:\Users\WangChi\AppData\Local\Temp\codex-clipboard-001137d8-10af-476f-b9ae-380ff3219e8f.png`

## 2. Target / Region

- Target：山水智鉴｜上海城市内涝智能防控中心首页核心视觉骨架与可复用 React 组件
- Region：1920×1080 Dashboard 全页；左侧 `StatusPanel` 当前积水态势、顶部导航、中央 DigitalTwinScene、选中事件浮层、右侧三面板、底部 Timeline；Gallery 中的单组件与组合状态
...
- `StatusPanel` 按本轮目标图重排为环形积水事件总数/一小时变化、三类处置状态卡、高发区域 TOP3 条形排行和四项 summary 指标；API 模式消费 `waterloggingSituation` 归一化结果，fixture 模式使用明确标注的 demo-only summary projection 供本地 review。
...
- `StatusPanel` 的视觉目标为用户本轮提供的图2：源文件 `C:\Users\WangChi\AppData\Local\Temp\codex-clipboard-94f3bfbf-5f6b-4292-9aa2-a91ed80e0342.png`，对照为图1 `C:\Users\WangChi\AppData\Local\Temp\codex-clipboard-001137d8-10af-476f-b9ae-380ff3219e8f.png`；目标状态 `default / summary data`。图2底部严重点位、`≥50cm` 和近24h字段不在当前 summary 契约中，未伪造。
...
| 用户积水态势参考图 | `StatusPanel` | 当前积水态势组件 | 1448 × 1086 | default / summary data | VISUAL_REVIEW |
